...
class MY_Output extends CI_Output {
    ...
    /**
     *
     * @var type representa outros dados que serão passados para o template
     */
    ...
    /**
     * @author Joas Ramos
     * @param type $data outros dados passados para template. Obs.: dados podem vir do servidor
     */
    function set_data($data) {

        if (array_key_exists('serie_a', $data)) {
            $this->other_data['serie_a'] = $data['serie_a'];
        }

        if (array_key_exists('competicao', $data)) {
            $this->other_data['competicao'] = $data['competicao'];
        }
    }
...
//		get_instance()->session->set_flashdata("__messages", serialize($this->_messages));
    ...
    private function get_template_output($output) {
        ...
        if (function_exists("get_instance") && class_exists("CI_Controller")) {
            ...
            /**
             * @author Joás Ramos <joasramos@uol.com.br>
             */
            $data['other_data'] = array();
            $data['other_data'] = $this->other_data;
